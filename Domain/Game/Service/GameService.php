...
final class GameService {
  ...
  public static function nextGameRound(Game $game): Game {
    ...
    try {
      DB::beginTransaction();
      $extraTime = max(0, 18 - $game->current_round * 2);
      ...
      $game = GameUpdater::fromId(id: $game->id, lockForUpdate: true)
        ...
        ->update();
      DB::commit();
      GameBroadcast::gameRoundUpdate(gameId: $game->id, roundNumber: $game->current_round, gameStateEnum: GameStateEnum::IN_PROGRESS);
      return $game;
    } catch (Throwable $e) {
      DB::rollBack();
      BearErrorCreator::create(message: "Failed to update game state [{$e->getMessage()}]", severity: BearSeverityEnum::CRITICAL, exception: $e);

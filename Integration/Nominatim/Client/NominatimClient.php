...
final class NominatimClient {
   ...
   */
  public static function lookup(int $osm_relation_id, string $type = 'R', bool $polygon = true, bool $extraTags = true, bool $nameDetails = false): array {
    $client = BearExternalApiClient::fromEnum(enum: BearExternalApiEnum::NOMINATIM);
    $data = $client->requestJsonOrThrow(path: "lookup", query: [
      'format' => 'jsonv2',
      'osm_ids' => "$type$osm_relation_id",

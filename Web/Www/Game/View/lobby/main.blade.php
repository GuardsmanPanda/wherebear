...
<x-panel class="m-2">
  <div class="flex flex-col w-full">
    <div class="flex gap-2">
      ...
      <img class="h-8 ml-auto rounded border border-shade-border-default shadow" src="https://devfuntime.gman.bot/static/flag/svg/{{ $user->cca2 }}.svg" alt="{{ $user->country_name }}" title="{{ $user->country_name }}" />
    </div>
    <div class="flex flex-col">
  ...
  <x-slot:heading>
    <x-heading label="Player Settings">
      <div class="flex items-center gap-2">
        <div class="text-md {{ $user->is_ready ? 'text-primary-text' : 'text-error-text' }} font-medium">{{ $user->is_ready ? 'Ready' : 'Not Ready' }}</div>
        <div class=" flex w-6 h-6 rounded-full border {{ $user->is_ready ? 'bg-primary-surface-default border-primary-border-default' : 'bg-error-surface-default border-error-border-default' }}"></div>
      </div>
    </x-heading>
    </x-slot>
    ...
    <div class="flex w-full gap-2">
      <div class="flex justify-between gap-2">
        <x-button-selector label="Marker" imageUrl='/static/img/map-marker/{{ $user->map_marker_file_name }}' hx-get="/game/{{$game->id}}/lobby/dialog/map-marker" />
        <x-button-selector label="Map" :imageUrl="MapStyleEnum::from($user->map_style_enum)->mapTileUrl(z: 11, x: 1614, y: 1016)" hx-get="/game/{{$game->id}}/lobby/dialog/map-style" />
      </div>
  ...
  <x-slot:heading>
    ...
    <div class="flex flex-col w-full gap-4">
      ...
      <div class="flex items-end gap-4 pb-4 border-b border-shade-border-light">
        <x-button label="Delete" :type="ButtonType::SECONDARY" :style="ButtonStyle::ERROR" :size="ButtonSize::SM" hx-delete="/game/{{$game->id}}" class="w-full" />
        <x-button label="Configure" :type="ButtonType::SECONDARY" :style="ButtonStyle::WARNING" :size="ButtonSize::SM" hx-get="/game/{{$game->id}}/lobby/dialog/settings" class="w-full" />
        <x-button label="Start" :type="ButtonType::SECONDARY" :style="ButtonStyle::INFO" :size="ButtonSize::SM" hx-post="/game/{{$game->id}}/start" class="w-full" />
      </div>
      @endif
      ...
      <div class="flex justify-between items-center gap-2">
        ...
        <div class="flex flex-col items-center w-full h-full gap-2 @if(!$loop->last) pr-2 border-r border-shade-border-light @endif">
          <span class="flex w-full h-full justify-center items-center font-heading text-sm text-shade-text-subtitle bg-tertiary-surface-default rounded">{{ $gameSetting['title'] }}</span>

...
<div x-data="state({{ $is_dev ?? null }})" class="flex flex-col h-screen bg-iris-200 select-none">
  <!-- Header -->
  ...
  <lit-round-result-header x-show="page === 'round'" x-cloak
  ...
  <!-- Stats Panel -->
  <lit-panel x-show="page === 'game'" label="STATS" class="mt-2 mx-2">
    <!-- Rank -->
  ...
  <div class="flex flex-col flex-1 min-h-0">
    <lit-panel label="Ranking" class="overflow-y-auto gap-2 m-2" x-show="page === 'game'">
    ...
    <div x-data x-show="page === 'round'"  class="w-full h-full relative bg-gray-600">
      <div 
        id="panorama" 
        class="w-full h-full absolute transition-opacity duration-500 ease-in-out"
        :class="{ 
          'opacity-0': mode !== 'panorama',
          'z-10': mode === 'panorama'
        }">
      </div>
      <lit-game-round-map
  ...
  <lit-round-list 
    ...
    :selectedRoundNumber="selectedRound.number"
    class="border-t border-gray-700 bg-iris-500"
    roundClickable
    x-on:clicked="selectRound($event)"
  ></lit-round-list>
</div>
  ...
  document.addEventListener('alpine:init', () => {
    Alpine.data('state', (isDev) => ({
      ...
      selectedRound: {
        ...
        user: @json($user),
        get userGuess() {
          return this.guesses.find(n => n.user_id === this.user.id);
        }
      },
      navigateHome() {
      ...
      navigateGame() {
        this.page = 'game';
        this.selectedRound.number = null;
      },
      switchMode(e) {
        this.mode = e.detail.isSelected ? 'map' : 'panorama';
      },
      async getRoundData(gameId, roundNumber) {
        if (isDev) {
          return {
            guesses: [
        ...
        const res = await fetch(`/web-api/game/${gameId}/round/${roundNumber}`, {
          method: 'GET',
          headers: {
      ...
      async selectRound(e) {
        ...
        const roundNumber = e.detail.roundNumber;
        const selectedRound = await this.getRoundData(this.game.id, roundNumber);     

        this.selectedRound.guesses = selectedRound.guesses;
        this.selectedRound.number = roundNumber;
        this.selectedRound.panorama.lat = selectedRound.panorama.lat;
        this.selectedRound.panorama.lng = selectedRound.panorama.lng;
        this.selectedRound.round = selectedRound.round;
        
        const sceneId = this.viewer.getScene();
        this.viewer.addScene(roundNumber, {
          panorama: selectedRound.panorama.url,
          yaw: selectedRound.panorama.heading,
          pitch: selectedRound.panorama.pitch,
          hfov: selectedRound.panorama.field_of_view
        });
        this.viewer.loadScene(roundNumber);
        if (sceneId) {
          this.viewer.removeScene(sceneId);
        }
      },
      init() {

    ...
    return html`
      <div class="flex flex-col w-full">
        ...
        <div class="flex justify-center gap-4 w-full mt-4">
          <lit-button-checkbox label="ALL" size="sm" class="w-full min-[500px]:w-28" .isDisabled="${this.isAllFilterTypeSelected}" .isSelected="${this.isAllFilterTypeSelected}" @clicked="${(e: CustomEvent) => this.selectAllFilterType(e.detail.isSelected)}"></lit-button-checkbox>
          ${AchievementProgressList.FILTER_TYPES.map(filterType => html`
            <lit-button-checkbox label="${filterType}" size="sm" class="w-full min-[500px]:w-28" .isSelected="${this.isFilterTypeSelected(filterType)}" @clicked="${(e: CustomEvent) => this.selectFilterType(filterType, e.detail.isSelected)}"></lit-button-checkbox>
          `)}
        </div>

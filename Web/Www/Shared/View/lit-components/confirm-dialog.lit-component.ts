...
class ConfirmDialog extends LitElement {
  ...
  protected render() {
    return html` <lit-dialog .label="${this.label}" modal @closed="${this.cancel}">
      <div slot="content" class="flex flex-col max-w-80 gap-8 mx-2 my-4">
        ...
        <div class="flex justify-center items-center gap-8">
          ...
          <lit-button .label="${this.confirmBtnText}" .bgColorClass="${this.confirmBtnBgColorClass}" class="min-w-20" @click="${this.confirm}">
          </lit-button>
        </div>
      </div>
    </lit-dialog>`

...
class EditGameSettingsDialog extends LitElement {
  ...
  protected render() {
    return html` <lit-dialog label="Game Settings" modal x-on:closed="closeEditGameSettingsDialog()">
      <div slot="content" class="flex flex-col mx-2">
        ${this.errorMsg
          ? html`<div class="text-poppy-700">${this.errorMsg}</div>`
          : html` <form class="flex flex-col" autocomplete="off">
              ...
              <fieldset class="flex gap-3 px-3 border border-gray-400 pb-3 mt-2">
                ...
                <label class="font-bold"
                  ...
                  <input
                    type="radio"
                    name="game-public-status-enum"
                    value="PUBLIC"
                    class="ml-1"
                    ?checked="${this.gamePublicStatusEnum === "PUBLIC"}"
                  />
                </label>
                ${this.isBob
                  ? html` <label class="font-bold"
                      >Google
                      <input
                        type="radio"
                        name="game-public-status-enum"
                        value="GOOGLE"
                        class="ml-1"
                        ?checked="${this.gamePublicStatusEnum === "GOOGLE"}"
                      />
                    </label>`
                  : nothing}
                <label class="font-bold"
                  ...
                  <input
                    ...
                    value="PRIVATE"
                    class="ml-1"
                    ?checked="${this.gamePublicStatusEnum === "PRIVATE"}"
                  />
                </label>
              </fieldset>
            </form>`}

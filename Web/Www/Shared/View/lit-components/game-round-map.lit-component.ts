...
export class GameRoundMap extends LitElement {
  ...
  private areCoordinatesInBounds(map: MapLibre, coordinates: LngLat[], padding = { top: 0, right: 0, bottom: 0, left: 0 }): boolean {
    ...
    const paddedBounds = new LngLatBounds(
      [bounds.getWest() - paddingLng.left, bounds.getSouth() - paddingLat.bottom],
      [bounds.getEast() + paddingLng.right, bounds.getNorth() + paddingLat.top],
    )

    return coordinates.every((coord) => paddedBounds.contains(coord))
  }

  private centerToPanoramaMarker() {
    const coordinatesToDisplay = this.guesses.slice(0, 3).map((guess) => new LngLat(guess.lng, guess.lat))

    const minZoom = 3
    const padding = { top: -100, right: -70, bottom: -100, left: -70 }

    while (!this.areCoordinatesInBounds(this.map, coordinatesToDisplay, padding) && this.map.getZoom() > minZoom) {
      this.map.setZoom(Math.max(this.map.getZoom() - 0.1, minZoom))
    }
  }
    ...
    this.guesses.forEach((guess) => {
      ...
      mapPlayerMarkerElement.innerHTML = `
        <lit-map-marker 
          ...
          flagCca2="${guess.country_cca2}" 
          flagDescription="${guess.country_name}" 
          flagFilePath="${guess.flag_file_path}" 
          iconFilePath="${guess.map_marker_file_path}" 
          playerName="${guess.user_display_name}" 
          rank="${guess.rank}">
        </lit-map-marker>
      `

      const marker = new Marker({
        element: mapPlayerMarkerElement,
        anchor: "bottom",
      })
    ...
    const map = new MapLibre({
      ...
      style: {
        ...
        sources: {
          "raster-tiles": {
            type: "raster",
            tiles: [this.mapStyleFullUri],
            tileSize: this.mapStyleTileSize,
          },
        },
        layers: [
          {
  ...
  updated(changedProperties: Map<string | number | symbol, unknown>) {
    ...
    if (changedProperties.has("guesses")) {
      this.clearPlayerMarkers()
      this.setPlayerMarkers()
    }

    if (changedProperties.has("panoramaLng") || changedProperties.has("panoramaLat")) {
      this.clearPanoramaMarker()
      if (this.panoramaLng && this.panoramaLat) {
        this.setPanoramaMarker(this.panoramaLng, this.panoramaLat)
      }
    }

    if (changedProperties.has("guesses") || changedProperties.has("panoramaLng") || changedProperties.has("panoramaLat")) {
      this.jumpToPanoramaMarker()
      this.centerToPanoramaMarker()
    }
  }

  protected render() {
    return html` <div id="map" class="w-full h-full"></div> `
  }
}

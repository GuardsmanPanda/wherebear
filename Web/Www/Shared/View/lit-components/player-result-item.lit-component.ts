  ...
  get rankTemplate() {
    ...
    return html`
      <div class="flex items-center">
        <img
          src="${rankIcons[this.rankIconType][rank]}"
          alt="${this.rankIconType === "cup" ? `${rank} place cup` : `${rank} place medal`}"
          class="${this.rankIconType === "cup" ? "h-8" : "h-[44px]"}"
        />
      </div>
    `
  }
  ...
  get scoreOnlyTemplate() {
    return html`
      <div class="flex justify-between items-center w-14 sm:w-[72px] mr-4">
        ...
        <div
          class="font-heading text-xl sm:text-2xl font-semibold text-[#F5D83A] text-stroke-2 text-stroke-gray-700"
          ${tooltip(this.detailedPoints || "")}
        >
          ${this.roundedPoints}
        </div>
      </div>

...
export class ToastTemplateBuilder {
  ...
  get contentClasses() {
    ...
    if (!this.#title && !this.#hasCloseButton) {
      return `${baseClasses} ${this.#iconPath ? 'rounded-r border-t border-r border-b' : 'rounded border'}`;
    }

    if (this.#title || this.#hasCloseButton) {
      return `${baseClasses} ${this.#iconPath ? 'rounded-br border-r border-b' : 'rounded-b border-r border-b border-l'}`;
    }
  ...
  build() {
    ...
    return `
      <div class="flex w-full rounded" style="box-shadow: 2px 2px 2px rgba(0, 0, 0, 0.15);">
        ${this.#iconPath ? `
        <div class="flex justify-center items-center flex-grow w-24 rounded-l border border-r-0 border-gray-700 bg-gray-600">
          <img src="${this.#iconPath}" class="w-16" />
        </div>
        ` : ''}
        <div class="flex flex-col w-full h-full rounded-r">
          ${this.#title || this.#hasCloseButton ? `
            <div class="flex shrink-0 ${this.#title ? 'justify-between' : 'justify-end'} items-center gap-4 h-10 pl-2 pr-1 rounded-tr border-t border-r ${!this.#iconPath ? 'border-l rounded-tl' : ''} ${!this.#content ? 'border-b rounded-b' : ''} border-gray-700 bg-gray-500">
